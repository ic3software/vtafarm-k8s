...
TEMPLATE_DIR="${REPO_ROOT}/stacks/05-vtafarm-app/_template"
CLUSTERS_DIR="${REPO_ROOT}/stacks/05-vtafarm-app/clusters"
...
if [ ! -d "$RKE2_DIR" ]; then
  ...
  echo "the app installs into that cluster; create it first with:" >&2
...
for config_file in backend.tf.example main.tf; do
  ...
  sed \
    ...
    -e 's|source = "../../../modules/vtafarm-app"|source = "../../../../modules/vtafarm-app"|' \
    "${TARGET_DIR}/${config_file}" >"$temporary_file"
  mv "$temporary_file" "${TARGET_DIR}/${config_file}"
done

echo "created ${TARGET_DIR}"
echo "next: fill in ${TARGET_DIR}/terraform.tfvars"
echo "then:  make init-vtafarm-app CLUSTER=${CLUSTER_NAME} && make apply-vtafarm-app CLUSTER=${CLUSTER_NAME}"
